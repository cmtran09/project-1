document.addEventListener("DOMContentLoaded", () => {
    ...
    const makeArenaArr = (width, height) => {
        for (let i = 0; i < height; i++) {
            //iterate, add an empy array each for the width
            tetrisArena[i] = [];
            for (let j = 0; j < width; j++) {
                // add a zero for each row
                tetrisArena[i][j] = 0;
            }
        }
        //add an empy arrray at bottom to allow for space for floor peices (bottom of arena)
        tetrisArena.push([]);
        for (let i = 0; i < width; i++) {
            tetrisArena[tetrisArena.length - 1].push(1);
        }
    };
    console.log(tetrisArena);

    makeArenaArr(width, height);
    ...
    let currentTetrimino = tetriminosArray[randomIndex][0];
    ...
    //     for (let j = 0; j < currentTetrimino.matrix[0][i].length; j++) {
    //       for (let k = 1; k < currentTetrimino.matrix[0][i][j].length; k++) {
    //         if (currentTetrimino.matrix[0][i][j][0] === 1) {
    //           squares[spawnPoint + currentTetrimino.matrix[0][i][j][k]].classList.add("block");
    //           console.log(currentTetrimino.color);
    //           //add classlist of colour
    //           squares[spawnPoint + currentTetrimino.matrix[0][i][j][k]].classList.add(
    ...
    //     for (let j = 0; j < currentTetrimino.matrix[0][i].length; j++) {
    //       for (let k = 1; k < currentTetrimino.matrix[0][i][j].length; k++) {
    //         squares[position + currentTetrimino.matrix[0][i][j][k]].classList.remove("block");
    //         console.log(currentTetrimino.color);
    //         //add classlist of colour
    //         squares[position + currentTetrimino.matrix[0][i][j][k]].classList.remove(
    //           currentTetrimino.color
    //         );
    //       }
    //     }
    ...
    const rotate90 = () => {
        ...
        if (currentRotation === currentTetrimino.matrix[0].length) {
            ...
            //   generate();
            //   return;
        } else currentTetrimino.matrix[0] = currentTetrimino.matrix[currentRotation];
        generate();
    };
    ...
    // const hardDrop = () => {
    //     clear()
    //     while (currentTetrimino.matrix[0].some(
    //             elem =>
    //             squares[position + elem + width].classList.contains("the-floor") !== true ||
    //             squares[position + elem + width].classList.contains("freeze") !== true
    //         )) {
